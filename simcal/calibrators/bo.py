...
class BayesianOptimization(sc.Base):
    ...
    def add_param(self, name: str, parameter: Ordered | Categorical):
        ...
        if isinstance(parameter, Linear):
            ...
            self._ordered_params[name] = cs.Float(name, (parameter.start, parameter.end), distribution=cs.Uniform(), log=False)
            ...
            self._ordered_params[name] = cs.Integer(name, (parameter.start, parameter.end), distribution=cs.Uniform(), log=True)
        else:
            self._categorical_params[name] = cs.Categorical(name, parameter.get_categories(), ordered=False)
        return self

    def calibrate(self, simulator: Simulator, early_stopping_loss=None, iterations=-1, timelimit=None, coordinator=None):
        if self._ordered_params == [] and self._categorical_params == []:
            raise ValueError(f"Missing hyperparameters")

        self.problem.add_hyperparameters(list(self._ordered_params.values()) + list(self._categorical_params.values()))
